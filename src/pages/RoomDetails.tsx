...
export const RoomDetails = () => {
  ...
  const { rooms, isLoading, fetchRooms } = useSiteStore();
  ...
  const room = rooms.find(r => r.seoSlug === slug);

  if (isLoading) {
    return (
      <div className="bg-pearl min-h-screen flex flex-col items-center justify-center space-y-4">
        <Loader2 className="w-12 h-12 text-gold animate-spin" />
        <p className="text-navy-500/80 font-medium animate-pulse font-body">Loading your room details...</p>
      </div>
    );
  }
  ...
  const galleryImages = room.images.filter((img: any) => !img.isFeatured).map((img: any) => img.url);
  const actualPrice = room.pricing?.standard;
  const offerPrice = room.pricing?.custom || actualPrice;

  return (
    <div className="bg-pearl min-h-screen">
      <SEOHead page={`room-${room.seoSlug}`} />
      <RoomSchema roomSlug={room.seoSlug || room.id} />
      ...
      {room.id === 'suite' ? (
        <section className="relative h-[100vh] w-full bg-navy overflow-hidden flex items-center justify-center">
          ...
          <div className="relative z-20 text-center flex flex-col items-center mt-20 px-6 w-full max-w-5xl">
            ...
            >
              {room.title.split(' ')[0]} <span className="italic text-gold font-light">{room.title.split(' ').slice(1).join(' ')}</span>
            </motion.h1>
            ...
            >
              <span className="flex items-center gap-2 md:gap-3"><span className="w-1 md:w-1.5 h-1 md:h-1.5 rounded-full bg-gold shadow-[0_0_8px_rgba(212,175,55,0.8)]" /> {room.area}</span>
              ...
              <span className="flex items-center gap-2 md:gap-3"><span className="w-1 md:w-1.5 h-1 md:h-1.5 rounded-full bg-gold shadow-[0_0_8px_rgba(212,175,55,0.8)]" /> {room.beds}</span>
            </motion.div>
          </div>
          ...
          <div className="absolute inset-y-0 left-0 p-8 md:p-16 max-w-7xl mx-auto z-20 flex flex-col justify-center h-full w-full md:w-2/3">
            ...
            >
              ...
              <h1 className="font-display text-display-md md:text-display-2xl text-white mb-6 drop-shadow-2xl leading-[0.9]">
                {room.title.split(' ')[0]} <br />
                <span className="italic text-gold font-light">{room.title.split(' ').slice(1).join(' ')}</span>
              </h1>
              
              <p className="font-body text-body-lg text-white/90 mb-10 max-w-xl drop-shadow-md border-l-2 border-gold pl-6 py-2">
                {room.subtitle || `${room.description.substring(0, 100)}...`}
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-6 text-xs md:text-body-sm uppercase tracking-widest text-pearl font-medium">
                <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md px-6 py-3 rounded border border-white/10">
                  <span className="w-1.5 h-1.5 rounded-full bg-gold" /> {room.area}
                </div>
                <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md px-6 py-3 rounded border border-white/10">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8 md:p-16 max-w-7xl mx-auto z-20 flex flex-col justify-end h-full">
            ...
            >
              ...
              <div className="flex flex-wrap gap-4 md:gap-8 text-xs md:text-body-sm uppercase tracking-widest text-pearl font-medium bg-black/20 backdrop-blur-md border border-white/10 py-3 px-6 rounded-full w-fit">
                <span className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-gold" /> {room.area}</span>
                <span className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-gold" /> Up to {room.capacity} Guests</span>
                <span className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-gold" /> {room.beds}</span>
              </div>
            </motion.div>
          </div>
        </section>
      ...
      <section className="py-20 max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-16 relative">
        ...
        <div className="lg:col-span-2">
          <div className="hidden md:block -mt-16 mb-8">
            <Breadcrumbs items={[{ label: 'Rooms', path: '/rooms' }, { label: room.title, path: `/rooms/${room.seoSlug}` }]} />
          </div>
          ...
          >
            <div className="mb-8">
              ...
              <div className="flex items-end gap-3">
                <span className="font-display text-display-sm text-gold leading-none">₹{offerPrice}</span>
                {offerPrice < actualPrice && (
                  <span className="text-body-sm text-navy-500/50 mb-1 line-through">₹{actualPrice}</span>
                )}
              </div>
            ...
            <a 
              href={`https://wa.me/${CONTACT_CONFIG.whatsapp.replace(/[^0-9]/g, '')}?text=Hi, I would like to book the ${room.title} at ₹${offerPrice}/night.`} 
              target="_blank" 
              rel="noreferrer"
            >
            ...
            <div className="p-4 bg-navy/5 border border-navy/10 rounded-sm">
              <p className="text-body-sm text-navy mb-3 flex items-start gap-2">
                ...
                <span>Offer price is for <strong>up to {room.capacity} guests</strong>.</span>
              </p>
              <p className="text-body-sm text-navy mb-3 flex items-start gap-2">
                <span className="text-gold mt-1 text-xs">◆</span> 
