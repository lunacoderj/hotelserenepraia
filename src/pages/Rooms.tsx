...
import { RoomsListSchema } from '../components/common/SchemaMarkup';
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-20 relative">
          <div className="absolute left-0 top-0 hidden md:block">
            <Breadcrumbs items={[{ label: 'Rooms', path: '/rooms' }]} />
          </div>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-32 space-y-4">
            ...
            <p className="text-navy-500/80 font-medium animate-pulse font-body">Loading available rooms...</p>
          </div>
        ) : (
                  ...
                  <LuxuryCard className="group h-full flex flex-col">
                    <div className="relative h-[400px] overflow-hidden">
                      ...
                      <div className="absolute top-6 right-6 bg-navy/90 backdrop-blur-sm px-6 py-3 text-pearl shadow-xl">
                        <span className="font-display text-heading-md block leading-none text-gold">₹{room.pricing?.custom || room.pricing?.standard}</span>
                        {room.pricing?.custom < room.pricing?.standard && (
                          <span className="font-body text-[10px] uppercase tracking-widest text-pearl/50 line-through">₹{room.pricing?.standard}</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="p-8 md:p-10 flex flex-col flex-grow bg-white relative z-10 -mt-4 mx-4 shadow-luxury">
                      <div className="flex justify-between items-end mb-4">
                        <h3 className="font-display text-heading-lg text-navy">{room.title}</h3>
                      ...
                      <ul className="grid grid-cols-2 gap-y-3 gap-x-4 mb-8">
                        {room.amenities.slice(0, 4).map((feature: string) => (
                      ...
                      <div className="pt-6 border-t border-navy/5 flex gap-4">
                        <Link to={`/rooms/${room.seoSlug}`} className="flex-1">
